...
- **🌉 [NeuronBridge Integration](docs/core-features/NeuronBridge_Guide.md)**: Bidirectional mapping between EM reconstructions and LM driver lines
  ...
  - **Specificity/Selectivity Analysis**: Use `NeuronBridge_Colabel.py` for detailed analysis of how specific each driver line is to your target neuron types (expression matrices, co-labeling similarity, labeling distributions)
...
vs = VisualizeSkeleton(
    ...
    merge_neurons = True,  # Also controls legend behavior (True='merge', False='normal')
    show_fig = True,
)
```
...
**Windows:**
```bash
# Windows users: Use the Windows-specific requirements file
pip install -r requirements-windows.txt
pip install neuronbridge-python --no-deps
```

> 💡 **Why Windows needs special handling:** The `neuronbridge-python` package on PyPI includes `memray` (a memory profiler) as a dependency, which only supports Linux/macOS. The `requirements-windows.txt` file excludes `memray` while including all other necessary dependencies. The two-step installation ensures NeuronBridge works properly on Windows.
