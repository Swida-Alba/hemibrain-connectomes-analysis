...
$DrocatVersion = "4.4.5"
$VersionLine = Select-String -Path "$ScriptDir\pyproject.toml" -Pattern '^version = "([^"]+)"' -ErrorAction SilentlyContinue
...
Invoke-InEnvironment @("python", "-m", "pip", "install", "--upgrade", "-r", "requirements-windows.txt")
...
Write-Host "Run: conda run -n $script:EnvName python scripts\FindDirect.py"
Write-Host "Configure NeuPrint/CAVE tokens in token_info_local.txt before online workflows."
