...
### 1. CSV Format
The CSV format is the most straightforward way to define mappings. It requires a `std_label` column and one column per dataset.
...
```
*   **std_label**: The standardized name used in your analysis.
...
### 2. JSON Format
The JSON format allows for defining both source and target mappings in a single file and supports nested structures.
...
mapper_json = LabelMapper(
    label_mapping_file='examples/data/example_label_mapping.json'
)
# or 
mapper_json = LabelMapper(
