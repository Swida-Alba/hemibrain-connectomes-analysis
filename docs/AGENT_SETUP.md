...
DROCAT v4.4.5 is the standalone-script edition. A coding agent can run the
prepared scripts, inspect result files, make a focused patch, and explain what
happened without a web interface. This guide assumes no prior agent experience.
...
> Install DROCAT v4.4.5 on this machine, verify `pip check`, and finish by reporting the environment status. I will run scripts directly.

The direct-analysis workflow expects the `drocat-4.4.5` Python 3.11
...
```text
Use the DROCAT v4.4.5 direct-analysis skill. Reproduce this direct-script
