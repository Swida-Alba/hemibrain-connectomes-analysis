...
**Feature**: VisualizePath now accepts connection matrices as input

**Supported formats**:
- Square matrices (NxN): e.g., neuron-to-neuron connectivity
- Rectangular matrices (NxM): e.g., source population to target population
- With or without index/column names (auto-generates N0, N1, ... if needed)
...
# Create connection matrix (10 sources x 12 targets)
matrix = pd.DataFrame(
...
# Visualize - automatically detects matrix format
vp = VisualizePath(matrix, showfig=True)
vp.visualize()
```

**Detection**:
- Prints format detection message: `✓ Recognized input format: connection matrix (10x12 DataFrame)`
- Automatically converts to edge-list internally
- Filters out zero and NaN connections

### 2. Individual Visualization Control

**Feature**: `visualize()` method now accepts parameters to control which visualizations to generate

**Parameters**:
- `plot_heatmap` (bool, default=True): Generate heatmap
- `plot_Sankey` (bool, default=True): Generate Sankey diagram
- `plot_network` (bool, default=True): Generate network graph
...
# Generate only network and Sankey (skip heatmap)
vp.visualize(plot_heatmap=False, plot_Sankey=True, plot_network=True)

# Generate all (default behavior)
vp.visualize()  # or explicitly: visualize(True, True, True)
```

**Benefits**:
- Faster workflow when only specific visualizations are needed
- Saves disk space and processing time
- Flexible for different use cases (e.g., heatmap for publications, network for exploration)

### 3. Enhanced Format Detection

**Feature**: Clear console messages indicating recognized input format

**Messages**:
- Connection matrix: `✓ Recognized input format: connection matrix (10x12 DataFrame)`
- Edge-list: `✓ Recognized input format: edge-list DataFrame (columns: ['source', 'target', 'weight'])`
- Path-based: `✓ Recognized input format: generic DataFrame (columns: ['path_block', 'weights'])`

**Edge-list detection supports**:
- Standard: `{source, target, weight}`, `{from, to, weight}`, `{pre, post, weight}`
- Prefixed: any columns ending with `_pre` and `_post` plus `weight`
- Examples: `{bodyId_pre, bodyId_post, weight}`, `{type_pre, type_post, weight}`

---

## Changes Made

### 4. Added `output_format` Attribute
