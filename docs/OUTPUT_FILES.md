...
### Folder Structure
The output folder name includes the function identifier, dataset, query and
parameters used:
`find-paths-complete_{DATASET}_{source}_to_{target}_L{max_interlayer}w{min_weight}r{min_ratio}p{min_prob}_{timestamp}`
(`find-paths-shortest_...` for shortest-path mode, `finddirect_...` for direct
connections).
...
### Folder Structure
Outputs are saved in a timestamped folder: `plot-3d_{layer_names}_{timestamp}`.
...
`homologs_{source_dataset}_to_{target_dataset}_{query}_{timestamp}/`
...
`cross-dataset_{source}_to_{target}_{dataset_codes}_{timestamp}/`
...
#### FindLines Mode (`NB-find-lines_{type}_{timestamp}/`)
...
#### FindNeuron Mode (`NB-find-neurons_{line}_{timestamp}/`)
...
*   **`plot-3d_{dataset}/`**: 3D skeleton visualizations per dataset.

#### Co-Labeling Mode (`NB-colabeling_{lines}_{timestamp}/`)
...
*   **`plot-3d_{dataset}/`**: Per-dataset 3D skeleton visualizations of matched neurons:
    *   `plot-3d_FAFB_flywire_FAFB_v783/`
    *   `plot-3d_MCNS_male-cns_v0_9/`
    *   `plot-3d_HEMI_hemibrain_v1_2_1/`
