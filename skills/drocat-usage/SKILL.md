---
name: drocat-usage
description: Directly run, explain, debug, and safely modify DROCAT v4.4.5 standalone connectome-analysis scripts without a web UI. Use when an agent needs to perform pathfinding, direct-connection analysis, cross-dataset comparison, homolog or connectivity-profile analysis, NeuronBridge/FlyLight searches, PlotPath or 3D skeleton visualization, empty-network generation, output inspection, or targeted backend fixes in this repository.
---

# DROCAT Direct Analysis

Use this skill to operate the v4.4.5 standalone backend as a script-driven research
...
1. Confirm the repository root and branch before running anything:
   ...
   Use branch `v4.4.5`. If the checkout is elsewhere, resolve that path first;
   do not clone a second copy over an existing project.

2. Use Python 3.11 in the `drocat-4.4.5` environment. Prefer
   `conda run -n drocat-4.4.5 ...` for a reproducible non-interactive command;
...
```bash
PYTHONNOUSERSITE=1 conda run -n drocat-4.4.5 python - <<'PY'
import inspect
