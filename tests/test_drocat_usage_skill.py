"""Offline checks for the v4.4.5 direct-analysis skill."""
...
def test_readme_exposes_direct_analysis_and_beginner_setup() -> None:
    ...
    assert "deepseek-v4-flash" in setup
    assert "raw.githubusercontent.com/Swida-Alba/Drosophila-cross-dataset-connectome-analysis/v4.4.5/skills/drocat-usage/SKILL.md" in readme
...
def test_skill_installation_uses_agent_command_not_manual_copy() -> None:
    ...
    setup = (ROOT / "docs" / "AGENT_SETUP.md").read_text(encoding="utf-8")
    raw_url = "raw.githubusercontent.com/Swida-Alba/Drosophila-cross-dataset-connectome-analysis/v4.4.5/skills/drocat-usage/SKILL.md"
...
def test_agent_workflows_have_completion_contracts() -> None:
    ...
    install_yaml = (ROOT / "skills" / "drocat-install" / "agents" / "openai.yaml").read_text(encoding="utf-8")
    setup = (ROOT / "docs" / "AGENT_SETUP.md").read_text(encoding="utf-8")
